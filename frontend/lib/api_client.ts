...
// Escrow API functions (for future modules)
...
export const submitWork = async (escrowId: number, data: {
  deliveryHash: string;
  walletAddress: string;
}) => {
  const response = await api.post(`/escrow/${escrowId}/submit`, data);
  return response.data;
};
...
export const raiseDispute = async (escrowId: number, walletAddress: string) => {
  const response = await api.post(`/escrow/${escrowId}/dispute`, { walletAddress });
  return response.data;
};
...
export const getUserEscrows = async (address: string, type: 'client' | 'freelancer') => {
  const response = await api.get(`/escrow/user/${address}?type=${type}`);
  return response.data;
};
